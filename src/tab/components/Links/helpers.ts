...
export function getPageTitle(doc: Document): string {
  return doc.querySelector('title') ? doc.querySelector('title').innerText : 'Unknown'
}
...
export async function saveCollectionToChromeStorage(c: ICollection) {
  console.time(`LINK:HELPER:: Adding/Updating collection to ChromeStorage ${c._id}`)
  ...
  console.timeEnd(`LINK:HELPER:: Adding/Updating collection to ChromeStorage ${c._id}`)
}
...
export async function addToCollection(hash: string, collection: ICollection): Promise<ICollection> {
  if (!collection.links.includes(hash)) {
    ...
    console.time(`LINK:HELPER:: Adding link, hash ${hash}, to collection ${collection._id}`)
    await store.put(newCollection, { pin: true })
    console.timeEnd(`LINK:HELPER:: Adding link, hash ${hash}, to collection ${collection._id}`)
