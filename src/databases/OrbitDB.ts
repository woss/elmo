 ...
 */
async function getCreatedStoresToChromeStorage(): Promise<IDatabaseDefinition[]> {
  return await getValuesByKey('createdDBs')
}

export function generateStoreName(name: string, dbNamePrefix = 'elmo', separator = '.'): string {
...
export function removeDbPrefix(name: string, dbNamePrefix = 'elmo', separator = '.'): string {
  const prefix = [dbNamePrefix, separator].join('')
  if (name.includes(prefix)) {
 ...
 * @param s
 */
export function transformStoreToElmoDefinition(s: IOrbitDBStoreType): IDatabaseDefinition {
  const ret = {
 ...
 * @param dbs
 */
export function createStoreDefinitions(dbs: IOrbitDBStoreType[]): IDatabaseDefinition[] {
  return Object.values(dbs).map((s) => transformStoreToElmoDefinition(s))
}

const ops = [
  ['meta_lens_info', [['meta_copyright', [['perceptual_hash', [['meta_create_date']]]]]]],
...
export async function createStores(
  ...
  return Promise.all(
    ...
    dbs.map((db) => instance[db.storeType](remote ? db.address : db.dbName, !remote || db.options)),
  ).then(async (stores) => {
 ...
 * @param name
 */
export async function openStoreByName(name: string): Promise<IOrbitDBStoreType> {
 ...
 */
export async function openStore(d: IDatabaseDefinition): Promise<IOrbitDBStoreType> {
  const { instance } = useDBNode()
 ...
 * @param dbs Array<IDatabaseDefinition>
 */
export async function openStores(dbs: IDatabaseDefinition[]): Promise<IOrbitDBStoreType[]> {
...
export function withStore(name: string): IOrbitDBStoreType {
  ...
  if (!R.isEmpty(storeInstance)) {
    ...
    if (store) {
      ...
      throw new Error(`Store :: ${name} :: doesn't exist. Did you forget to open it?`)
    }
  } else {
    throw new Error('No connected OrbitDB nodes.')
  }
...
export async function addCollection(c: ICollection, pin = true): Promise<ICollection> {
